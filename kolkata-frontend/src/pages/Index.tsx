...
import api, { SiteStat } from "@/api/client";

const fallbackIcons = [Building2, Award, Users];
...
const Index = () => {
  ...
  const parseSuffix = (val: string) => val.replace(/[0-9]/g, "");
          ...
          >
            ...
            >
              <Link to="/projects" className="relative inline-block text-lg group">
                ...
                <span className="absolute bottom-0 right-0 w-full h-12 -mb-1 -mr-1 transition-all duration-200 ease-linear rounded-full group-hover:mb-0 group-hover:mr-0"></span>
              </Link>

              <Link
                to="/contact"
                className="relative px-5 py-3 overflow-hidden font-medium text-gray-800 bg-gray-100 bg-opacity-30 backdrop-blur-lg border border-gray-100 rounded-lg shadow-inner group"
              >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="font-display text-4xl font-bold mb-1">
                      <CountUpValue targetValue={parseNumeric(stat.statValue)} />
                      <span>{parseSuffix(stat.statValue)}</span>
                    </div>
                    <div className="text-muted-foreground text-sm">{stat.statLabel}</div>
                  </motion.div>
                );
              })}
          </div>
        ...
        <div className="container mx-auto px-6 text-center">
          ...
          >
            ...
            >
              <Link
                to="/contact"
                className="relative inline-flex items-center justify-center p-4 px-6 py-3 overflow-hidden font-medium text-accent transition duration-300 ease-out border-2 border-accent rounded-full shadow-md group"
              >
